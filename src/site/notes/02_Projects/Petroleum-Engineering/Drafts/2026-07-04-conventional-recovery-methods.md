...
| 问题            | 名字                                                                             | 白话            |
| ------------- | ------------------------------------------------------------------------------ | ------------- |
| 地下**原来有多少油**？ | **OOIP**（original oil in place）/ 有时写 STOIIP（stock tank oil-initially-in-place） | 「油罐」里一开始装了多少油 |
| 其中**能采出多少**？  | reserves / 可采储量 ≈ OOIP × 采收率                                                   | 真正能卖的那部分      |

一次采出「约 10%–30% OOIP」、二次再采「约 10%–20% OOIP」，说的都是：**相对「原来有多少」的比例**，不是相对 reserves。
...
| 符号 | 英文 | 中文 | 含义 |
|---|---|---|---|
| $N$ | OOIP / STOIIP | 原始地质储量 | 结果，单位通常是 **STB**（地面油罐桶） |
| $7758$ | — | 换算系数 | 1 acre·ft 的体积 ≈ **7758 barrel**（把岩石体积单位换成桶） |
| $A$ | area | 含油面积 | 油藏平面面积（acre） |
| $h$ | net pay thickness | 有效厚度 | 真正含油的那一层有多厚（ft）；$A \times h$ = 岩石体积（acre·ft） |
| $\phi$ | porosity | 孔隙度 | 孔隙占岩石体积的比例，用小数（0.2 = 20%） |
| $S_w$ | water saturation | 含水饱和度 | 孔隙流体中水的比例；**油占的是 $1-S_w$** |
| $B_{oi}$ | oil FVF at initial conditions | 初始原油体积系数 | 地下 1 桶油体积，对应地面多少桶？$B_{oi}$ 通常 **> 1**（例如 1.2），所以要**除以**它 |

课里口述的 $V_o = V \phi S_h$、$S_h = 1 - S_w$，就是上式的「分子部分」：先算**地下孔隙里的油体积**；再除以 $B_{oi}$，才得到地面 STB。
...
| OOIP | 地下原来有多少油（地面 STB 口径）；$N=7758Ah\phi(1-S_w)/B_{oi}$ | 有效厚度如何划定 |
